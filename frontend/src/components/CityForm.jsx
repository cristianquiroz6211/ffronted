...
const CityForm = () => {
  ...
  const fetchStates = async () => {
    ...
    try {
      console.log("Realizando solicitud GET a https://localhost:8080/general/api/v1/states...");
      const response = await axios.get('https://localhost:8080/general/api/v1/states', {
        headers: {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      >
        {loading ? 'Creating...' : 'Crear Ciudad'}
      </button>
      <button onClick={() => logout({ returnTo: window.location.origin })} className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
        Log Out
      </button>
    </form>
  );
